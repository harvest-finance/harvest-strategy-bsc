...
const VaultMigratable_Pancake_CAKE_BNB = artifacts.require("VaultMigratable_Pancake_CAKE_BNB");
const VaultProxy = artifacts.require("VaultProxy");

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Pancake CAKE_BNB Migration", function() {
  let accounts;
  ...
  async function setupExternalContracts() {
    underlying = await IBEP20.at("0xA527a61703D82139F8a06Bc30097cC9CAA2df5A6");
  ...
  before(async function() {
    ...
    newImplementation = await VaultMigratable_Pancake_CAKE_BNB.new({
      gas: 5000000
    });

    vault = await Vault.at("0xfFBD102fAFbd9e15C9122d9C62aB299AFD4D3E4F");
    vaultAsNewImplementation = await VaultMigratable_Pancake_CAKE_BNB.at("0xfFBD102fAFbd9e15C9122d9C62aB299AFD4D3E4F");
  ...
  describe("Happy path", function() {
    it("Upgrade must succeed and Farmer should earn money", async function() {
     ...
     await Utils.waitHours(13);
      ...
      const f00dCAKE = await cake.balanceOf(governance);
      const f00dBNB = await bnb.balanceOf(governance);

      console.log("f00dCAKE: ", f00dCAKE.toString());
