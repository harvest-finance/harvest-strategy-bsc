  ...
  */
  ...
  function redeemPartialWithLoan(
    ...
    while (borrowed > newBorrowTarget) {
      ...
      uint256 toRedeem = MathUpgradeable.min(supplied.sub(requiredCollateral), amount.add(toRepay));
      _redeemUnderlying(toRedeem);
      // now we can repay our borrowed amount
      uint256 underlyingBalance = IBEP20(_underlying()).balanceOf(address(this));
      _repay(MathUpgradeable.min(toRepay, underlyingBalance));
  ...
  function redeemMaximumWBNBWithLoan(uint256 collateralFactorNumerator, uint256 collateralFactorDenominator) internal {
    // amount of liquidity in Venus
    uint256 available = CompleteVToken(vToken()).getCash();
    // amount we supplied
    uint256 supplied = CompleteVToken(vToken()).balanceOfUnderlying(address(this));
  ...
  function redeemPartialWBNBWithLoan(
    uint256 amount,
    uint256 borrowTargetFactorNumerator,
    uint256 collateralFactorNumerator,
    uint256 factorDenominator) internal {
    ...
    while (borrowed > newBorrowTarget) {
      ...
      // now we can repay our borrowed amount
      uint256 underlyingBalance = IBEP20(_underlying()).balanceOf(address(this));
      _repayInWBNB(MathUpgradeable.min(toRepay, underlyingBalance));
      // update the parameters
      borrowed = CompleteVToken(vToken()).borrowBalanceCurrent(address(this));
      supplied = CompleteVToken(vToken()).balanceOfUnderlying(address(this));
    }
