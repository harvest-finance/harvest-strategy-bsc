...
contract ValueMasterChefStrategyLP is BaseUpgradeableStrategy {
  using SafeMath for uint256;
  using SafeBEP20 for IBEP20;

  address constant public valueLiquidRouter = address(0xb7e19a1188776f32E8C2B790D9ca578F2896Da7C);
  address constant public vBSWAP = address(0x4f0ed527e8A95ecAA132Af214dFd41F30b361600);
  ...
  function initializeStrategy(
    ...
    address _lpt;
    (_lpt,,,,) = IRewardPool(rewardPool()).poolInfo(_poolID);
  ...
  function rewardPoolBalance() internal view returns (uint256 bal) {
      (bal,) = IRewardPool(rewardPool()).userInfo(poolId(), address(this));
  }
  ...
  function enterRewardPool() internal {
    ...
    IRewardPool(rewardPool()).deposit(poolId(), entireBalance);
  }
  ...
  */
  function emergencyExit() public onlyGovernance {
    IRewardPool(rewardPool()).emergencyWithdraw(poolId());
  ...
  */
  ...
  function _liquidateReward() internal {
    uint256 vBSWAPBalance = IBEP20(vBSWAP).balanceOf(address(this));
    if (!sell() || vBSWAPBalance < sellFloor() || vBSWAPBalance == 0) {
      // Profits can be disabled for possible simplified and rapid exit
      emit ProfitsNotCollected(sell(), vBSWAPBalance < sellFloor());
      return;
    }

    //swap vBSWAP to reward token on valueswap
    IBEP20(vBSWAP).safeApprove(valueLiquidRouter, 0);
    IBEP20(vBSWAP).safeApprove(valueLiquidRouter, vBSWAPBalance);
  ...
  */
  function withdrawAllToVault() public restricted {
    if (address(rewardPool()) != address(0)) {
      ...
      IRewardPool(rewardPool()).withdraw(poolId(), bal);
    }
    _liquidateReward();
  ...
  */
  function withdrawToVault(uint256 amount) public restricted {
    ...
    if(amount > entireBalance){
      ...
      IRewardPool(rewardPool()).withdraw(poolId(), toWithdraw);
    }
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    uint256 bal = rewardPoolBalance();
    if (bal != 0) {
      IRewardPool(rewardPool()).withdraw(poolId(), 0);
      _liquidateReward();
    }
    investAllUnderlying();
  }
