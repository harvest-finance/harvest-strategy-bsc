...
import "../interface/mdex/IMdexPair.sol";
import "../interface/mdex/IMdexRouter.sol";

contract MdexMasterChefStrategy is BaseUpgradeableStrategy {
  using SafeMath for uint256;
  using SafeBEP20 for IBEP20;

  address constant public pancakeswapRouterV2 = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
  address constant public mdexRouter = address(0x7DAe51BD3E3376B8c7c4900E9107f12Be3AF1bA8);
  address constant public mdx = address(0x9C65AB58d8d978DB963e63f2bfB7121627e3a739);
  ...
  //Reward token will be WBNB. Need pancake liquidity for fee notification.
  address[] public mdexMDX2BNB;
  ...
  function initializeStrategy(
    ...
    address _lpt;
    (_lpt,,,,,) = IBSCPool(rewardPool()).poolInfo(_poolID);
    ...
    if (_isLpToken) {
      ...
      mdexRoutes[uniLPComponentToken0] = new address[](0);
      mdexRoutes[uniLPComponentToken1] = new address[](0);
    } else {
      pancakeswapRoutes[underlying()] = new address[](0);
      mdexRoutes[underlying()] = new address[](0);
    }
  ...
  function rewardPoolBalance() internal view returns (uint256 bal) {
      (bal,,) = IBSCPool(rewardPool()).userInfo(poolId(), address(this));
  }
  ...
  function enterRewardPool() internal {
    ...
    IBSCPool(rewardPool()).deposit(poolId(), entireBalance);
  }
  ...
  */
  function emergencyExit() public onlyGovernance {
    IBSCPool(rewardPool()).emergencyWithdraw(poolId());
  ...
  */
  ...
  // set liquidation path from reward to underlying.
  // if _pancake = false, store it as mdexRoute
  function setLiquidationPath(address _token, address [] memory _route, bool _pancake) public onlyGovernance {
    ...
    if (_pancake) {
      ...
      mdexRoutes[_token] = _route;
    }
  }

  // We assume that all the tradings can be done on Pancakeswap
  function _liquidateReward(uint256 mdxBalance) internal {
    // Swap MDX to WBNB to notify profit fee
    IBEP20(mdx).safeApprove(mdexRouter, 0);
    IBEP20(mdx).safeApprove(mdexRouter, mdxBalance);

    // we can accept 1 as minimum because this is called only by a trusted role
    uint256 amountOutMin = 1;

    IMdexRouter(mdexRouter).swapExactTokensForTokens(
      mdxBalance,
      amountOutMin,
      mdexMDX2BNB,
      address(this),
    ...
    if (isLpAsset()) {
      ...
      if (pancakeswapRoutes[uniLPComponentToken0].length > 1) {
        // allow Pancakeswap to sell our reward
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, 0);
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, toToken0);
      ...
      } else if (mdexRoutes[uniLPComponentToken0].length > 1){
        // allow Mdex to sell our reward
        IBEP20(rewardToken()).safeApprove(mdexRouter, 0);
        IBEP20(rewardToken()).safeApprove(mdexRouter, toToken0);
        // if we need to liquidate the token0
        IMdexRouter(mdexRouter).swapExactTokensForTokens(
          toToken0,
          amountOutMin,
          mdexRoutes[uniLPComponentToken0],
          address(this),
          block.timestamp
        );
        token0Amount = IBEP20(uniLPComponentToken0).balanceOf(address(this));
      ...
      if (pancakeswapRoutes[uniLPComponentToken1].length > 1) {
        // allow Pancakeswap to sell our reward
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, 0);
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, toToken1);
      ...
      } else if (mdexRoutes[uniLPComponentToken1].length > 1){
        // allow Mdex to sell our reward
        IBEP20(rewardToken()).safeApprove(mdexRouter, 0);
        IBEP20(rewardToken()).safeApprove(mdexRouter, toToken1);
        // if we need to liquidate the token0
        IMdexRouter(mdexRouter).swapExactTokensForTokens(
          toToken1,
          amountOutMin,
          mdexRoutes[uniLPComponentToken1],
          address(this),
          block.timestamp
        );
        token1Amount = IBEP20(uniLPComponentToken1).balanceOf(address(this));
      ...
      // provide token0 and token1 to MDEX
      IBEP20(uniLPComponentToken0).safeApprove(mdexRouter, 0);
      IBEP20(uniLPComponentToken0).safeApprove(mdexRouter, token0Amount);

      IBEP20(uniLPComponentToken1).safeApprove(mdexRouter, 0);
      IBEP20(uniLPComponentToken1).safeApprove(mdexRouter, token1Amount);

      // we provide liquidity to Pancake
      uint256 liquidity;
      (,,liquidity) = IMdexRouter(mdexRouter).addLiquidity(
      ...
      if (pancakeswapRoutes[underlying()].length > 1) {
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, 0);
        IBEP20(rewardToken()).safeApprove(pancakeswapRouterV2, remainingRewardBalance);
        IPancakeRouter02(pancakeswapRouterV2).swapExactTokensForTokens(
      ...
      } else if (mdexRoutes[underlying()].length > 1){
        IBEP20(rewardToken()).safeApprove(mdexRouter, 0);
        IBEP20(rewardToken()).safeApprove(mdexRouter, remainingRewardBalance);
        IMdexRouter(mdexRouter).swapExactTokensForTokens(
          remainingRewardBalance,
          amountOutMin,
          mdexRoutes[underlying()],
          address(this),
          block.timestamp
        );
      }
  ...
  */
  function withdrawAllToVault() public restricted {
    if (address(rewardPool()) != address(0)) {
      ...
      IBSCPool(rewardPool()).withdraw(poolId(), bal);
    }
    if (underlying() != rewardToken()) {
      uint256 mdxBalance = IBEP20(mdx).balanceOf(address(this));
      if (mdxBalance != 0) {
        _liquidateReward(mdxBalance);
      }
    }
  ...
  */
  function withdrawToVault(uint256 amount) public restricted {
    ...
    if(amount > entireBalance){
      ...
      IBSCPool(rewardPool()).withdraw(poolId(), toWithdraw);
    }
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    uint256 bal = rewardPoolBalance();
    if (bal != 0) {
      uint256 mdxBalanceBefore = IBEP20(mdx).balanceOf(address(this));
      IBSCPool(rewardPool()).withdraw(poolId(), 0);
      uint256 mdxBalanceAfter = IBEP20(mdx).balanceOf(address(this));
      uint256 claimedMdx = mdxBalanceAfter.sub(mdxBalanceBefore);
      if (claimedMdx != 0) {
        _liquidateReward(claimedMdx);
      }
    }
    investAllUnderlying();
  }
