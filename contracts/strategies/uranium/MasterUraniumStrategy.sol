...
import "../../base/masterchef/interface/IMasterChef.sol";
import "../../base/interface/IStrategy.sol";

import "./interface/ISRadsToken.sol";
...
contract MasterUraniumStrategy is BaseUpgradeableStrategy {
    ...
    address constant public sRads = address(0x28d2E3BB1Ec54A6eE8b3Bee612F03A85d3Ec0C0c); //U235 == SRads
    address constant public rads = address(0x670De9f45561a2D02f283248F65cbd26EAd861C8);  //U92  == Rads
    ...
    address[] public rads2rewardPath;
    ...
    function initializeStrategy(
        ...
        rads2rewardPath = [rads, rewardToken()];
    }
    ...
    */
    ...
    function _liquidateReward(uint256 rewardBalance) internal {
        ...
        // Convert sRADS (U235) into RADS (U92)
        ISRadsToken(sRads).swapToRads(rewardBalance);
        uint256 radsBalance = IBEP20(rads).balanceOf(address(this));

        // Swap to BUSD for notification
        IBEP20(rads).safeApprove(uraniumRouter, 0);
        IBEP20(rads).safeApprove(uraniumRouter, radsBalance);
    ...
    */
    function withdrawAllToVault() public restricted {
        ...
        if (underlying() != sRads) {
            uint256 rewardBalance = IBEP20(sRads).balanceOf(address(this));
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        uint256 bal = rewardPoolBalance();
        if (bal != 0) {
            uint256 rewardBalanceBefore = IBEP20(sRads).balanceOf(address(this));
            IMasterChef(rewardPool()).withdraw(poolId(), 0);
            uint256 rewardBalanceAfter = IBEP20(sRads).balanceOf(address(this));
