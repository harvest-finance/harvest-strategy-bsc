...
import "../../base/uranium-base/interface/IUraniumMasterChefDepositFee.sol";
import "../../base/interface/IStrategy.sol";

import "./interface/IU235Token.sol";
...
contract MasterUraniumStrategy is BaseUpgradeableStrategy {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    address constant public pancakeswapRouterV2 = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    address constant public uraniumRouter = address(0xF4EE46Ac2BA83121F79c778ed0D950ffF11a18Ed);
    address constant public u235 = address(0x28d2E3BB1Ec54A6eE8b3Bee612F03A85d3Ec0C0c);
    address constant public u92 = address(0x670De9f45561a2D02f283248F65cbd26EAd861C8);
    ...
    address[] public U235_2rewardPath;
    ...
    function initializeStrategy(
        ...
        address _lpt;
        (_lpt,,,,,,) = IUraniumMasterChefDepositFee(rewardPool()).poolInfo(_poolID);
        ...
        U235_2rewardPath = [u92, rewardToken()];
    }
    ...
    function rewardPoolBalance() internal view returns (uint256 bal) {
        (bal,,) = IUraniumMasterChefDepositFee(rewardPool()).userInfo(poolId(),address(this));
    }
    ...
    function enterRewardPool() internal {
        uint256 depositFee;
        (,,,,,depositFee,) = IUraniumMasterChefDepositFee(rewardPool()).poolInfo(poolId());
        ...
        IUraniumMasterChefDepositFee(rewardPool()).deposit(poolId(), entireBalance);
    }
    ...
    */
    function emergencyExit() public onlyGovernance {
        ...
        IUraniumMasterChefDepositFee(rewardPool()).withdraw(poolId(), bal);
    ...
    */
    ...
    function _liquidateReward(uint256 rewardBalance) internal {
        if (!sell() || rewardBalance < sellFloor() || rewardBalance == 0) {
            // Profits can be disabled for possible simplified and rapid exit
            emit ProfitsNotCollected(sell(), rewardBalance < sellFloor());
            return;
        }
        // Convert U235 into U92
        IU235Token(u235).swapToRads(rewardBalance);
        uint256 u92Balance = IBEP20(u92).balanceOf(address(this));

        // Swap to BUSD for notification
        IBEP20(u92).safeApprove(uraniumRouter, 0);
        IBEP20(u92).safeApprove(uraniumRouter, u92Balance);

        // we can accept 1 as minimum because this is called only by a trusted role
        uint256 amountOutMin = 1;
        IPancakeRouter02(uraniumRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            u92Balance,
            0,
            U235_2rewardPath,
            address(this),
        ...
        uint256 busdBalance = IBEP20(rewardToken()).balanceOf(address(this));

        notifyProfitInRewardToken(busdBalance);
        ...
        // allow Uranium to sell our reward
        IBEP20(rewardToken()).safeApprove(uraniumRouter, 0);
        IBEP20(rewardToken()).safeApprove(uraniumRouter, remainingRewardBalance);
        ...
        if (isLpAsset()) {
            ...
            IBEP20(uniLPComponentToken0).safeApprove(uraniumRouter, 0);
            IBEP20(uniLPComponentToken0).safeApprove(uraniumRouter, token0Amount);

            IBEP20(uniLPComponentToken1).safeApprove(uraniumRouter, 0);
            IBEP20(uniLPComponentToken1).safeApprove(uraniumRouter, token1Amount);

            // we provide liquidity to Pancake
            uint256 liquidity;
            (,,liquidity) = IPancakeRouter02(uraniumRouter).addLiquidity(
    ...
    */
    function withdrawAllToVault() public restricted {
        ...
        if (underlying() != u235) {
            uint256 rewardBalance = IBEP20(u235).balanceOf(address(this));
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > entireBalance){
            ...
            IUraniumMasterChefDepositFee(rewardPool()).withdraw(poolId(), toWithdraw);
        }
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        uint256 bal = rewardPoolBalance();
        if (bal != 0) {
            uint256 rewardBalanceBefore = IBEP20(u235).balanceOf(address(this));
            IUraniumMasterChefDepositFee(rewardPool()).withdraw(poolId(), 0);
            uint256 rewardBalanceAfter = IBEP20(u235).balanceOf(address(this));
