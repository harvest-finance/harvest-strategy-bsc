...
import "../../base/pancake-base/interface/IMasterChef.sol";
...
contract GooseStrategy is BaseUpgradeableStrategy {
  ...
  */
  ...
  function _liquidateLpAssets(uint256 remainingRewardBalance) internal {
    ...
    if (
      uniLPComponentToken1 == rewardToken()  // we are dealing with CAKE/WBNB LP
      && routesToken0.length > 1 // we have a route to do the swap
    ) {
      token1Amount = remainingRewardBalance / 2; // 1/2 of CAKE is saved for LP
      uint256 toToken0 = remainingRewardBalance.sub(token1Amount); // other 1/2 is liquidated
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    ...
    if (bal != 0) {
      ...
      rewardBalance = IBEP20(rewardToken()).balanceOf(address(this));
      _liquidateReward();
      rewardBalance = IBEP20(rewardToken()).balanceOf(address(this));
    }
