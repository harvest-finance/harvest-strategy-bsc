...
contract PopsicleStrategy is BaseUpgradeableStrategy {
  ...
  function initializeStrategy(
    ...
    if (_isLpToken) {
      ...
      sushiswapRoutes[underlying()] = new address[](0);
    }
  ...
  */
  ...
  function setLiquidationPath(address _token, address [] memory _route) public onlyGovernance {
    ...
    sushiswapRoutes[_token] = _route;
  }

  // We assume that all the tradings can be done on Pancakeswap
  function _liquidateReward(uint256 iceBalance) internal {
    if (!sell() || iceBalance < sellFloor()) {
      // Profits can be disabled for possible simplified and rapid exit
      emit ProfitsNotCollected(sell(), iceBalance < sellFloor());
      return;
    }
    //swap ice to bnb to notify
    IBEP20(ice).safeApprove(sushiswapRouter, 0);
    IBEP20(ice).safeApprove(sushiswapRouter, iceBalance);

    // we can accept 1 as minimum because this is called only by a trusted role
    uint256 amountOutMin = 1;

    IPancakeRouter02(sushiswapRouter).swapExactTokensForTokens(
      iceBalance,
      amountOutMin,
      ice2bnb,
      address(this),
    ...
    IBEP20(rewardToken()).safeApprove(sushiswapRouter, 0);
    IBEP20(rewardToken()).safeApprove(sushiswapRouter, remainingRewardBalance);
    ...
    if (isLpAsset()) {
      ...
      if (sushiswapRoutes[uniLPComponentToken0].length > 1) {
        // if we need to liquidate the token0
        IPancakeRouter02(sushiswapRouter).swapExactTokensForTokens(
          toToken0,
          amountOutMin,
          sushiswapRoutes[uniLPComponentToken0],
          address(this),
          block.timestamp
        );
      ...
      if (sushiswapRoutes[uniLPComponentToken1].length > 1) {
        // sell reward token to token1
        IPancakeRouter02(sushiswapRouter).swapExactTokensForTokens(
          toToken1,
          amountOutMin,
          sushiswapRoutes[uniLPComponentToken1],
          address(this),
          block.timestamp
        );
      ...
      IBEP20(uniLPComponentToken0).safeApprove(sushiswapRouter, 0);
      IBEP20(uniLPComponentToken0).safeApprove(sushiswapRouter, token0Amount);

      IBEP20(uniLPComponentToken1).safeApprove(sushiswapRouter, 0);
      IBEP20(uniLPComponentToken1).safeApprove(sushiswapRouter, token1Amount);

      // we provide liquidity to Pancake
      uint256 liquidity;
      (,,liquidity) = IPancakeRouter02(sushiswapRouter).addLiquidity(
  ...
  */
  ...
  function finalizeUpgrade() external onlyGovernance {
    ...
    if (isLpAsset()) {
      sushiswapRoutes[IPancakePair(underlying()).token0()] = new address[](0);
      sushiswapRoutes[IPancakePair(underlying()).token1()] = new address[](0);
    } else {
      sushiswapRoutes[underlying()] = new address[](0);
    }
  }
