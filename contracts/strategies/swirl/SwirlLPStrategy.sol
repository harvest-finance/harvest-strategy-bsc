...
import "./interface/ISwirlLiquidityMining.sol";
...
contract SwirlLPStrategy is BaseUpgradeableStrategy {
  ...
  function initializeStrategy(
    ...
    address _lpt;
    (_lpt,,) = ISwirlLiquidityMining(rewardPool()).liquidityMining();
  ...
  function rewardPoolBalance() internal view returns (uint256 bal) {
      (bal,,) = ISwirlLiquidityMining(rewardPool()).userInfo(address(this));
  }
  ...
  function enterRewardPool() internal {
    ...
    ISwirlLiquidityMining(rewardPool()).deposit(entireBalance);
  }
  ...
  */
  function emergencyExit() public onlyGovernance {
    ...
    ISwirlLiquidityMining(rewardPool()).withdraw(bal);
  ...
  */
  ...
  function _liquidateReward(uint256 rewardBalance) internal {
    ...
    // Convert to WBNB to notify
    IBEP20(swirl).safeApprove(pancakeswapRouterV2, 0);
    IBEP20(swirl).safeApprove(pancakeswapRouterV2, rewardBalance);
    ...
    uint256 wbnbBalance = IBEP20(rewardToken()).balanceOf(address(this));

    notifyProfitInRewardToken(wbnbBalance);
  ...
  */
  function withdrawAllToVault() public restricted {
    if (address(rewardPool()) != address(0)) {
      uint256 bal = rewardPoolBalance();
      ISwirlLiquidityMining(rewardPool()).claim();
      ISwirlLiquidityMining(rewardPool()).withdraw(bal);
    }
    uint256 rewardBalance = IBEP20(swirl).balanceOf(address(this));
  ...
  */
  function withdrawToVault(uint256 amount) public restricted {
    ...
    if(amount > entireBalance){
      ...
      ISwirlLiquidityMining(rewardPool()).withdraw(toWithdraw);
    }
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    uint256 bal = rewardPoolBalance();
    if (bal != 0) {
      uint256 rewardBalanceBefore = IBEP20(swirl).balanceOf(address(this));
      ISwirlLiquidityMining(rewardPool()).claim();
      uint256 rewardBalanceAfter = IBEP20(swirl).balanceOf(address(this));
