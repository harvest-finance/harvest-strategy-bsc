...
import "./interface/IBalloonFarm.sol";
...
contract DeflateStrategy is BaseUpgradeableStrategy {
  using SafeMath for uint256;
  using SafeBEP20 for IBEP20;

  address constant public pancakeswapRouterV2 = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
  address constant public bln = address(0x887bf46573b9a77c4060919E786B881f08f15de4);
  ...
  address[] public pancakeBLN2WBNB;
  ...
  function initializeStrategy(
    ...
    address _lpt;
    (_lpt,,,,) = IBalloonFarm(rewardPool()).poolInfo(_poolID);
  ...
  function rewardPoolBalance() internal view returns (uint256 bal) {
      (bal,) = IBalloonFarm(rewardPool()).userInfo(poolId(), address(this));
  }
  ...
  function enterRewardPool() internal {
    ...
    IBalloonFarm(rewardPool()).deposit(poolId(), entireBalance);
  }
  ...
  */
  function emergencyExit() public onlyGovernance {
    ...
    IBalloonFarm(rewardPool()).withdraw(poolId(), bal);
  ...
  */
  ...
  function _liquidateReward() internal {
    //Swap BLN to wbnb
    uint256 blnBalance = IBEP20(bln).balanceOf(address(this));
    // we can accept 1 as minimum because this is called only by a trusted role
    uint256 amountOutMin = 1;
    if (blnBalance > 0) {
      IBEP20(bln).safeApprove(pancakeswapRouterV2, 0);
      IBEP20(bln).safeApprove(pancakeswapRouterV2, blnBalance);
  ...
  */
  function withdrawToVault(uint256 amount) public restricted {
    ...
    if(amount > entireBalance){
      ...
      IBalloonFarm(rewardPool()).withdraw(poolId(), toWithdraw);
    }
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    uint256 bal = rewardPoolBalance();
    if (bal != 0) {
      IBalloonFarm(rewardPool()).withdraw(poolId(), 0);
      _liquidateReward();
    }
